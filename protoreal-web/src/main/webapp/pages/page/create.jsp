...
<html:form action="/PageCreate.do?method=create">
	...
	<div class="form-group">
		...
		<html:select property="pageType" styleClass="form-control">
			<html:optionsCollection name="pageForm" property="typeList"
				label="label" value="value" />
		</html:select>
	</div>

	<div class="form-group">
		<label for="inputProjectName"><bean:message
				key="label.containerType" /></label>
		<html:select property="containerType" styleClass="form-control">
			<html:optionsCollection name="pageForm" property="containerTypeList"
				label="label" value="value" />
		</html:select>
	</div>
	...
	<div class="form-group">
		<label for="inputProjectName"><bean:message
				key="label.pages.parent" /></label>
		<html:select property="parentPage" styleClass="form-control">
			<html:option value="0">
				<bean:message key="label.select" />
			</html:option>
			<html:optionsCollection name="pageForm" property="pageList"
				label="title" value="uuid" />
		</html:select>
	</div>
	...
	<div class="form-group" style="margin-left: 20px;">
		<html:checkbox styleClass="form-check-input" property="showTitle"
			onchange="changeCheckbox('checkShowTitle')" />
		<label for="inputProjectName"><bean:message
				key="label.pages.showTitle" /></label>
	</div>

	<div class="form-group" style="margin-left: 20px;">
		<html:checkbox styleClass="form-check-input" property="displayOnMenu" />
		<label for="inputProjectName"><bean:message
				key="label.pages.displayOnMenu" /></label>
	</div>

	<div class="form-group" style="margin-left: 20px;">
		<html:checkbox styleClass="form-check-input" property="hideMenu" />
		<label for="inputProjectName"><bean:message
				key="label.pages.hideMenu" /></label>
	</div>

	<div class="form-group">
		<label for="inputProjectDescription"><bean:message
				key="label.description" /></label>
		<html:textarea property="description" cols="5" rows="10"
